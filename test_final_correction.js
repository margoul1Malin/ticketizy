// Test final de la logique corrigée
const testData = [
    {
        model: 'OLED77C54LA',
        wo: '4EMGL11L',
...
console.log('=== TEST FINAL LOGIQUE CORRIGÉE ===');
...
// Tester la logique de génération d'étiquettes
console.log('\n=== TEST GÉNÉRATION D\'ÉTIQUETTES ===');
testData.forEach(result => {
    if (result.sn && result.model) {
        const wo = result.wo || null;
        if (wo) {
            console.log(`Étiquette complète: ${result.model} | ${result.sn} | W/O: ${wo}`);
        } else {
            console.log(`Étiquette sans W/O: ${result.model} | ${result.sn} | (Pas de ligne W/O)`);
        }
    }
});
